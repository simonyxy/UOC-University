...
function BaseClass(super)
	...
	-- 于是这么做的好处就现象出来了，类型Layer自身的键	值不会发生改变，因此可以做为一个“稳定”的基类去派生子类，而所有的变化，都体现在Layer所对应的虚表vtbl_Layer中，这样的设计，封装了变化，也便于继承的实现
...
-- 创建
BaseLevel = BaseLevel or BaseClass()
function BaseLevel:__init()
	print("~~")
end
function BaseLevel:__delete()
end
FirstLevel = FirstLevel or BaseClass(BaseLevel)
function FirstLevel:__init()
	print("~~~")
end
function FirstLevel:__delete( )
end
local test = FirstLevel.New()
test:DeleteMe()
test = nil